...
物理 CPU 中，每执行完一条指令，CPU 都将检查中断引脚是否激活，是则处理中断。中断芯片通过激活连接 CPU 的 INTR 引脚，通知 CPU 处理中断。

### PIC

1. 虚拟设备向虚拟中断芯片 PIC 发送中断请求
2. 唤醒 vCPU 对应的线程，加入到物理 CPU 的就绪队列
3. VM Entry 之前，KVM 检查 PIC 是否有中断要处理，触发虚拟 PIC 芯片的中断评估逻辑
4. 有需要 guest 处理的中断，向 VMCS VM-entry interruption-information 字段写入信息，注入中断
5. 进入 guest 模式，CPU 检查 VMCS 中断信息
6. 调用 guest IDT 中对于的中断服务处理

### APIC

与 PIC 的单处理器系统的不同点：

1. CPU 之间会收发中断 Inter-Processor Interrupt, IPI，每个 CPU 一个 LAPIC，都连接到统一的 I/O APIC 上，两个组件都要虚拟。
2. 唤醒 vCPU 线程，如果 vCPU 是在另一个物理 CPU 上运行 guest 模式，还需要向其发送 IPI，使其陷入到 host，然后在 VM entry 时进行中断注入。

在 guest 模式的 CPU 无法检查虚拟 APIC 的引脚，只能在 VM Entry 时由 KVM 检查，然后写入到 VMCS。也就是说，一旦需要中断注入，就需要触发 VM Exit 陷入。

Intel 提供硬件支持，减少陷入次数。之前 guest 访问 LAPIC 寄存器时，会触发 VM Exit，但这个过程其实不需要 VMM 介入。物理 LAPIC 上有一个页大小的内存用于存储寄存器，称为 APIC page，Intel 实现了 guest 模式的页，称为 virtual-APIC page。

此外，Intel 还在 guest 模式下实现了部分中断芯片的逻辑，如中断评估，称为虚拟中断逻辑。在 guest 模式下有了状态和逻辑，就可以模拟很多中断的行为，无需 VMM 接入，大大减少 VM Exit 的次数。中断评估即判断中断是否被屏蔽、多个中断请求的优先级等。

硬件虚拟化支持下，LAPIC 收到中断时，不必等到下一次 VM Entry 被动地执行中断评估，而是主动向 guest 模式的 CPU 告知，讲中断信息写入 posted-interrupt descripter。然后 LAPIC 通过特殊的核间中断 posted-interrupt notification 通知目标 CPU，目标 CPU 在 guest 模式下借助虚拟中断逻辑处理中断。
